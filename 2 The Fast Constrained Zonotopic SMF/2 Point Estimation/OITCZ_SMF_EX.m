...
%                 temp_range_os = zonotope_scaling(temp_range_os);
    ...
    cZ_posterior = OIT_inspired_estimation(A, B, C, temp_cZ_prior, cZ_w, cZ_v, y_sequence(:, k-delta_bar+kIndexC: k+kIndexC), k, delta_bar, column_scaling_flag, B_u, u_sequence(:, k-delta_bar+kIndexC: k-1+kIndexC), prior_refinement_flag); % One can make it faster, since G_hat, c_hat, and A_hat are unchanged; only parts of b_hat need to be replaced.
